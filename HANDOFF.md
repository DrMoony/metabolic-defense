...
- **퀴즈 세트 2종**: `MASLD/MASH`(AASLD 덱 46문) + `Clinical Obesity`(Lancet Commission 2025·대한비만학회 팩트시트/지침·AHA CKM 기반 38문), 각 한/영·난이도 3단
  - 학회 성격에 맞춰 비율 조절: 게임 시작화면에서 **A키 → 어드민**(MASLD 100 / 70:30 / 50:50 / 30:70 / Obesity 100)
  - **특정 약물(분자명·브랜드·약물시험) 문항 11문은 기본 숨김** — 공정경쟁규약 대응. 어드민에서 '포함'으로 켤 수 있음
...
## 웨이브 보스 (2026-09-02)
- 킹 버거 보스 삭제. **웨이브가 끝날 때마다 전용 보스 1기**가 마무리로 등장한다(`WAVE_BOSS`).
  | 웨이브 | 보스 | HP | 그 시점 무기 기준 |
  |---|---|---|---|
  | 1 (t=26) | 🛢️ 과당 시럽통 (`syrup`, 신규) | 16 | 화승총 8발 / 권총 8발 · 위쪽 밸브가 약점(2배) |
  | 2 (t=42) | 🧬 암세포 (`cancer`) | 26 | 샷건 6발 / 매그넘 9발 · 3조각으로 분열 |
  | 3 (t=48) | 🩸 죽상경화 플라크 (`plaque`) | 34 | 바주카 7발 / 레이저 9발 · 간을 뚫고 돌진 |
- **보스 전용 에너지 바**: 화면 하단 중앙(HUD와 겹치지 않게), 10칸 눈금·이름 표시·25% 이하에서 테두리 점멸.
  일반 몹의 작은 머리 위 바와 확실히 구분된다. `bossBarShow(e)` / `bossBarUpdate()`.
- 등장 시 화면 플래시·카메라 흔들림·저음 2연타로 긴장감. 난이도 자체는 그 시점 무기로 3~5초면 잡히는 수준.

## 신규 콘텐츠 (2026-08-31)
- **비행 몬스터 추가: 프라이드 치킨윙**(`wing`) — hp2·speed5.0·score340·liverX1.6, 우상단에서 날아옴. 3웨이브 13초부터 등장.
- **무기 12종으로 확장** — 샷건과 기관단총 사이에 **매그넘**(index 5) 삽입: dmg3·cd0.5·6연발·**관통 3**.
  관통 판정은 '맞은 대상보다 뒤 + 6.2 이내'로 계산한다(카메라가 내려다보는 각이라 탄선 거리로는 뒤쪽을 못 잡음).
  총 모델 티어 번호도 함께 밀렸다(기관단총6·소총7·기관총8·바주카9·유도미사일10·레이저11).
- **유도미사일**(index 10, 바주카↔레이저 사이): dmg4·cd0.32·8발·splash2.6(바주카보다 좁음)·homing.
  조준 지점 14 이내에서 가장 가까운 적을 락온(없으면 카메라에 가장 가까운 적), `r.target`을 매 프레임
  표적 쪽으로 lerp(turn 5.5)해 곡선 비행. 빗나가게 쏴도 표적을 따라감 — 실측 확인.
...
## 스프라이트 전면 교체 완료 (2026-09-06, beta)
ChatGPT(구독)에서 35종을 생성해 절차 메시를 전부 갈아끼웠다. 파이프라인·함정은 `tools/README.md`.
- 장기 2(간·췌장 이미지 교체) · 보스 3(시럽통·암세포·플라크) · 몬스터 13(지상 8=걷기 2프레임, 비행 4=날갯짓 2프레임, 버거 1프레임)
- 소품: 지방 둔덕, 덫 새장+빨간 자물쇠(투명 히트존 유지), GLP-1/GCGR 주사펜 드롭
